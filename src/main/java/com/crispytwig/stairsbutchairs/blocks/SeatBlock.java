...
import com.crispytwig.stairsbutchairs.registry.SBCTags;
...
//        if (isFakePlayer(player)) return InteractionResult.PASS;
    ...
    public static boolean isSeatBlocked(Level level, BlockPos pos) {
        return !(level.getBlockState(pos.above()).getCollisionShape(level, pos).isEmpty() ||
                level.getBlockState(pos.above()).is(SBCTags.BlockTags.ABOVE_BYPASSES_SEAT_CHECK) ||
    ...
    public static boolean canBePickedUp(Entity passenger) {
        ...
        return passenger instanceof LivingEntity && passenger.getType().is(SBCTags.EntityTags.CAN_SIT_IN_SEATS);
    }
